...
#define SERVO_SP_DATA_2D_BUF_NAME "sensorData2d"
...
/*! Class for representing 2D sensor return
  ...
  Modified to allow for map data as well as range data. Note that all
...
public:
  ...
  //! dimensions of map
  int y;
  //! minimum range of return for range data
  double minRange;
  //! maximum range of return for range data
  double maxRange;
  /*!
     Variable length array of feature values to add. The name of the array is
...
//! enum lookup function
extern const char *sensorData_enum_ServoSensorType_symbol_lookup(long v);
